...
## `equals`
<!-- TODO: Link to reference variables in the Data Types chapter 2.3.3.1 -->

Suppose we have two objects of type `Student`, say `student1` and `student2`, and we want to determine if they are equal. If we try to compare them using `==`, we will likely get a result we did not expect. This is because `student1` and `student2` are [reference variables](TODO), which means they hold a reference to, or the address of, the actual `Student` objects. `student1` and `student2` evaluate as equal only when they have the same memory address.
